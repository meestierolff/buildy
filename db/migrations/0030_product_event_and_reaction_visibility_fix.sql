-- Restore the original product-event trigger contract and re-apply the blocked
-- actor filter that moderation-policy rewrites dropped from reaction reads.
  ...
  AND (
    ...
    OR EXISTS (
      ...
      WHERE target_comment.id = reactions.comment_id
        AND target_comment.update_id = reactions.update_id
        AND target_comment.project_id = reactions.project_id
        AND NOT public.app_moderation_target_hidden('comment', target_comment.id)
        AND target_comment.status = 'published'
    )
  )
);
